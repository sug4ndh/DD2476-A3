...
//public class SearchGUI extends JFrame {
    ...
    ///**  The returned documents, used in search() and relevanceFeedbackSearch() */
    //private PostingsList results,tmp;
    //private PostingsList res1=new PostingsList(); 
    //private PostingsList res2=new PostingsList();  
	
    ///**  Directories that should be indexed. */
    //LinkedList<String> dirNames = new LinkedList<String>();
	
	///** Value of k for sub-phrase retrieval */
	//int k=200;
	...
	//Action search = new AbstractAction() {
		//public void actionPerformed( ActionEvent e ) {
			...
			//results = indexer.index.search( query, queryType, rankingType, structureType );
			//System.err.println("results "+results.Returnlist().size());
		//}
             //else if(structureType==Index.BIGRAM){
              //results = indexer.biindex.search( query, queryType, rankingType, structureType );
              //System.err.println("results "+results.Returnlist().size());}
              //else if(structureType==Index.SUBPHRASE){
							//boolean flag1 = true;
                            //res1 = indexer.biindex.search( query, queryType, rankingType, structureType );
                            //System.err.println("Performing biworded query...");
                            //tmp=res1;
                            
                            //if(res1.size()<=k){
							//flag1 = false;
							//System.err.println("Performing uniworded query now...");
                            //res2 = indexer.index.search( query, queryType, rankingType, structureType );
                            ////results = indexer.biindex.search( query, queryType, rankingType, structureType );
                            ////System.err.println("results "+results.Returnlist().size());
                            //System.err.println("res1 "+res2.Returnlist().size());
                            //for(PostingsEntry p1:res1.Returnlist()){
                                //for(PostingsEntry p2:res2.Returnlist()){
                            ...
                            //results = res2;
                             //tmp = res2;}
                            //if(flag1){System.err.println("Only biworded query performed");}
                            //HashMap<Integer, PostingsEntry> entries = new HashMap<Integer, PostingsEntry>();
                            //for(PostingsEntry t1:tmp.Returnlist()){
                                //if (!entries.containsKey(t1.docID)) {
                                    //entries.put(t1.docID, new PostingsEntry(t1.docID));
                                  //}
