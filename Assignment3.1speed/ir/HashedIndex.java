...
private PostingsList rankedcombination(Query q){
   ...
   LinkedList<String> terms = q.copy().terms;
	...
	private PostingsList cosineSim(Query n) {
	System.err.println("inside improved");
	
	final int N = docIDs.size();
	Query q = new Query();
	//LinkedList<String> terms = q.copy().terms;
	LinkedList<String> terms1 = n.copy().terms;
        ArrayList<PostingsList> postl = new ArrayList<>();
    ...
    //termarr empty
        ...
        for(String t:n.terms){
        System.err.println("term in q "+t);
        termarr.add(getPostings(t));
        }
        //weights(docLengths.size(),termarr);
        if(terms1.size()==0){
        ...
        HashMap<String, Integer> queryTermsnew = new HashMap<String, Integer>();
        for ( int i = 0; i < terms1.size(); i++ ) {
            String term = n.terms.get(i);
            if (queryTermsnew.containsKey(term)){
            //tf
            queryTermsnew.put(term, queryTermsnew.get(term)+1);
            } else {
            queryTermsnew.put(term, 1);
            }
        }
        
        for (String term : n.terms) {
            PostingsList pl = getPostings(term);
            if (pl == null) continue;
            int tf = queryTermsnew.get(term);
            final int df = pl.size();
            final double idf = Math.log10(((double) N) / df);
            if (idf > 0.6){
                q.terms.add(term);
